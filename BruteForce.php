...
class BruteForce {
    ...
    private static function _db() {
        ...
        if (sizeof(self::$databaseConfig) > 0) {
            $db = new \PDO(
                    "mysql:host=" . self::$databaseConfig['host'] . ";port=" . self::$databaseConfig['port'] . ";dbname=" . self::$databaseConfig['dbname'], self::$databaseConfig['username'], self::$databaseConfig['password']
            );
        } else {
